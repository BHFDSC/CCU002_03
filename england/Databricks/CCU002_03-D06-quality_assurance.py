...
preg = spark.sql("SELECT * FROM .ccu002_03_codelists WHERE name = 'pregnancy_and_birth'")
...
pros = spark.sql("SELECT * FROM .ccu002_03_codelists WHERE name = 'prostate_cancer'")
pros = [item.code for item in pros.select('code').collect()]

# COMMAND ----------

df_gdppr = spark.table(".ccu002_03_df_gdppr_sex") # created for rule 6, no need to re-do
