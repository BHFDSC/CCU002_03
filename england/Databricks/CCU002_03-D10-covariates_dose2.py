...
for codelist in medhistory:
   sql("CREATE OR REPLACE GLOBAL TEMP VIEW ccu002_03_cov_dose2_sus_" + codelist + " AS SELECT DISTINCT NHS_NUMBER_DEID FROM (SELECT data.NHS_NUMBER_DEID, data.EPISODE_START_DATE, vaccination.vaccination_dose1_date FROM .ccu002_03_vaccination AS vaccination INNER JOIN (SELECT NHS_NUMBER_DEID, EPISODE_START_DATE FROM .ccu002_03_sus_longformat WHERE ((CODE IN (SELECT code FROM .ccu002_03_codelists WHERE name = '" + codelist + "' and terminology=='ICD10')) OR (LEFT(CODE,3) IN (SELECT code FROM .ccu002_03_codelists WHERE name = '" + codelist + "' and terminology=='ICD10')))) AS data ON vaccination.NHS_NUMBER_DEID = data.NHS_NUMBER_DEID) WHERE EPISODE_START_DATE<vaccination_dose1_date")
...
# MAGIC       FROM .ccu002_03_vaccination AS vaccination 
# MAGIC       INNER JOIN (SELECT person_id_deid, date 
# MAGIC                   FROM .ccu002_03_ccu013_covid_trajectory 
...
# MAGIC                   FROM .ccu002_03_gdppr_) AS data 
