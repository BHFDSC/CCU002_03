...
for codelist in medhistory:
   sql("CREATE OR REPLACE GLOBAL TEMP VIEW ccu002_03_cov_dose1_hesapc_" + codelist + " AS SELECT DISTINCT NHS_NUMBER_DEID FROM (SELECT NHS_NUMBER_DEID, EPISTART FROM .ccu002_03_hes_apc_longformat WHERE CODE IN (SELECT code FROM .ccu002_03_codelists WHERE name = '" + codelist + "' AND TERMINOLOGY='ICD10')) WHERE EPISTART<'" + index_date + "'")
...
for codelist in medhistory:
   sql("CREATE OR REPLACE GLOBAL TEMP VIEW ccu002_03_cov_dose1_sus_" + codelist + " AS SELECT DISTINCT NHS_NUMBER_DEID FROM (SELECT NHS_NUMBER_DEID, EPISODE_START_DATE FROM .ccu002_03_sus_longformat WHERE ((CODE IN (SELECT code FROM .ccu002_03_codelists WHERE name = '" + codelist + "' and terminology=='ICD10')) OR (LEFT(CODE,3) IN (SELECT code FROM .ccu002_03_codelists WHERE name = '" + codelist + "' and terminology=='ICD10')))) WHERE EPISODE_START_DATE<'" + index_date + "'")
...
sql("CREATE OR REPLACE GLOBAL TEMP VIEW ccu002_03_cov_dose1_prior_covid19 AS SELECT DISTINCT person_id_deid AS NHS_NUMBER_DEID, 1 AS cov_dose1_prior_covid19 FROM (SELECT data.person_id_deid, data.date, vaccination.vaccination_dose1_date FROM .ccu002_03_vaccination AS vaccination INNER JOIN (SELECT person_id_deid, date FROM .ccu002_03_ccu013_covid_trajectory WHERE covid_status='confirmed') AS data ON vaccination.NHS_NUMBER_DEID = data.person_id_deid) WHERE date<'" + index_date + "' AND date>='2020-01-01'")
...
sql("CREATE OR REPLACE GLOBAL TEMP VIEW ccu002_03_cov_dose1_lsoa_nondistinct AS SELECT DISTINCT NHS_NUMBER_DEID, LSOA FROM (SELECT DISTINCT NHS_NUMBER_DEID, LSOA, DATE FROM .ccu002_03_gdppr_) WHERE DATE<'" + index_date + "'")
...
# MAGIC FROM .ccu002_03_lsoa_region_lookup
...
# MAGIC       WHERE LSOA_CODE_2011 IN (SELECT LSOA FROM .ccu002_03_gdppr_)
