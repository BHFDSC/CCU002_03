...
df$outcome <- factor(df$outcome, 
                       levels = c("myocarditis/pericarditis","myocarditis","pericarditis"),
                       labels = c("Myocarditis/pericarditis","Myocarditis","Pericarditis"))
...
ggplot2::ggplot(df, mapping = ggplot2::aes(x=days_post_vaccination, y=estimate, color=exposure)) +
  ...
  ggplot2::facet_wrap(outcome~dose, ncol = 2, scales = "free_x")

# Save figure ------------------------------------------------------------------

ggplot2::ggsave(filename = "output/figure_outcomes.jpeg",
                dpi = 600, width = 210, height = 210,
                unit = "mm", scale = 1)