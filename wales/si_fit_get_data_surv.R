...
fit_get_data_surv <- function(fml, vac_str, covars,  agegp, event, survival_data, cuts_weeks_since_expo){
  ...
  df_age_region <- cohort_agegp %>% dplyr::select(NHS_NUMBER_DEID, agegroup)
  ...
  with_expo <- tmerge(data1=d1, data2=d2, id=NHS_NUMBER_DEID,
                      ...
                      expo=tdc(day_expo)) 
  # with_expo <- with_expo %>%C dplyr::select(!id)
  ...
  get_tbl_event_count <- function(data_surv, interval_names){
    df_events <- data_surv %>% dplyr::filter(event==1)
  ...
  tbl_event_count_sex1 <- get_tbl_event_count(data_surv %>% dplyr::filter(SEX==1), interval_names)
  tbl_event_count_sex2 <- get_tbl_event_count(data_surv %>% dplyr::filter(SEX==2), interval_names)
  ...
  tbl_event_count <- tbl_event_count %>%
    dplyr::arrange(factor(expo_week, 
  ...
  # change file path fed in
  if (fml %in% c("+ ind_expo*agegroup + sex", "+ ind_expo*sex + agegroup")){
    write.csv(tbl_event_count, paste0("P:/torabif/workspace/CCU0002-03/results/tbl_event_count_ind_expo_" , expo, "_", event, "_", agegp, "_", vac_str,".csv"), row.names = F)
  } else {
    write.csv(tbl_event_count, paste0("P:/torabif/workspace/CCU0002-03/results/tbl_event_count_" , expo, "_", event, "_", agegp, "_", vac_str,".csv"), row.names = F)
  }
